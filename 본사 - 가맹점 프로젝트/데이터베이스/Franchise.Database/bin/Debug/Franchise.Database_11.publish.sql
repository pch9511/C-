...
IF EXISTS (select top 1 1 from [dbo].[Register])
    RAISERROR (N'행이 발견되었습니다. 데이터가 손실될 수 있으므로 스키마 업데이트가 종료됩니다.', 16, 127) WITH NOWAIT

GO
PRINT N'[dbo].[Login]에 대한 명명되지 않은 제약 조건 삭제 중...';


GO
ALTER TABLE [dbo].[Login] DROP CONSTRAINT [DF__Login__Check__68487DD7];


GO
PRINT N'[dbo].[Register]에 대한 명명되지 않은 제약 조건 삭제 중...';


GO
ALTER TABLE [dbo].[Register] DROP CONSTRAINT [DF__Register__Date__71D1E811];
...
GO
ALTER TABLE [dbo].[Login] DROP CONSTRAINT [FK__Login__RegiNum__6754599E];


GO
PRINT N'[dbo].[Login] 테이블 다시 빌드 시작...';
...
IF EXISTS (SELECT TOP 1 1 
           FROM   [dbo].[Login])
    BEGIN
        INSERT INTO [dbo].[tmp_ms_xx_Login] ([Id], [Password], [RegiNum], [Check])
        SELECT   [Id],
                 [Password],
                 [RegiNum],
                 [Check]
...
GO
PRINT N'[dbo].[Register] 변경 중...';


GO
ALTER TABLE [dbo].[Register] DROP COLUMN [Date];
...
DECLARE tableconstraintnames CURSOR LOCAL FORWARD_ONLY
    FOR SELECT SCHEMA_NAME([schema_id]),
               ...
               0
        FROM   [sys].[objects]
        WHERE  [parent_object_id] IN (OBJECT_ID(N'dbo.Login'), OBJECT_ID(N'dbo.ChatLog'), OBJECT_ID(N'dbo.FranStock'), OBJECT_ID(N'dbo.HeadStock'), OBJECT_ID(N'dbo.HeadStockOut'), OBJECT_ID(N'dbo.Order'))
               AND [type] IN (N'F', N'C')
